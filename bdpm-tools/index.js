...
const files = {
	...
	'CIS_InfoImportantes': {
		...
		headers: [
			{ name: 'cis', type: 'string', pattern: /^\d{8}$/ },
			{ name: 'dateDebutInfo', type: 'date', toFormat: 'DD/MM/YYYY', format: 'YYYY-MM-DD' },
			{ name: 'dateFinInfo', type: 'date', toFormat: 'DD/MM/YYYY', format: 'YYYY-MM-DD' },
	...
	try {
		...
		const p = json_formats
			...
			.map(async p => {
				...
				try {
					const res = await converter(path.join(__dirname, bdpm_folder), filename, files[file].headers, doc => {
